...
int  CtorArsCore (ArsCore* core, const char* ars_file) 
    ...
    // stat(ars_file, &text_data);
    // printf("File %s size: %zd\n", ars_file, text_data.st_size);
    ...
    //                         (text_data.st_size - sizeof(IntelIArsSignature))/sizeof(cmd_code) );
    ...
    StackCtor (&(core->CPU_stk), CPU_STACK_INITIAL_CAPACITY); // add checking stk ctor
    //$i((core->CPU_stk).top)
